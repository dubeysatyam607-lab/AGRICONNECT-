  ...
  const menuSections = [
    {
      ...
      items: [
        ...
        { icon: Shield, label: tr('profile.menu.kycVerification', 'Govt ID & KYC Verification'), sub: tr('profile.menu.kycVerificationSub', 'Aadhaar & Kisan Credit Card (KCC)'), tint: 'text-primary', action: () => setCurrentView('kyc-verification') },
    ...
    },
    {
      ...
      items: [
        { icon: LayoutDashboard, label: tr('profile.menu.adminConsole', 'Executive Admin Console'), sub: tr('profile.menu.adminConsoleSubtitle', 'KPI metrics & platform control'), tint: 'text-primary', action: () => { setActiveRole('Admin'); if (typeof window !== 'undefined') localStorage.setItem('agri_admin_session', 'true'); onNavigate('admin'); } },
  ...
  if (currentView === 'my-bookings') {
    return (
      <div className="pb-24 pt-4 px-4">
        ...
        <SidebarErrorBoundary>
          <AgriCard>
            <h3 className="font-semibold text-lg mb-4">My Bookings</h3>
          ...
          <AgriCard>
            ...
            <div className="space-y-4 text-sm">
              <div className="p-3 bg-muted rounded-lg">
                <h4 className="font-semibold mb-1">Call our Kisan Helpline</h4>
              ...
              <div className="mt-6 pt-4 border-t">
                <h4 className="font-semibold mb-2">Frequently Asked Questions (Offline Cached)</h4>
  ...
  if (currentView === 'farm-details') {
    ...
    return (
      <div className="pb-24 pt-4 px-4">
        ...
        <SidebarErrorBoundary>
          <AgriCard>
            <h3 className="font-semibold text-lg mb-1">Farm Details</h3>
            <p className="text-sm text-muted-foreground mb-4">Your Home dashboard uses these details to personalise advice.</p>
            <div className="grid grid-cols-2 gap-3">
              {statCards.map((s) => (
                <div key={s.label} className="rounded-xl border border-border bg-background/60 p-3.5">
                  <s.icon size={17} className="text-feature-mandi mb-2" />
                  <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{s.label}</p>
                  <p className="text-[15px] font-semibold text-foreground mt-0.5 truncate">{s.value}</p>
                </div>
              ))}
            </div>
            ...
            {/* Active crop picker */}
            <div className="mt-5">
              <p className="text-[12px] font-semibold text-foreground mb-2">Active Crop</p>
                ...
                {COMMON_CROPS.map((crop) => (
                  <button
                    ...
                    className={`shrink-0 rounded-full border px-3 py-1.5 text-[12px] font-semibold transition-all ${
                      farm.crop === crop
                        ? "border-feature-mandi bg-feature-mandi text-white "
            ...
            {/* Crop stage picker */}
            <div className="mt-4">
              <p className="text-[12px] font-semibold text-foreground mb-2">Crop Stage</p>
              <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
            ...
            {/* Farm area picker */}
            <div className="mt-4">
              <p className="text-[12px] font-semibold text-foreground mb-2">Farm Area (acres)</p>
            ...
            <button
              ...
              className="mt-5 w-full rounded-xl bg-primary text-primary-foreground py-2.5 font-semibold text-sm active:scale-[0.98] transition-transform"
            >
  ...
  if (isLoading || typeof user === 'undefined') {
    return (
      <div className="pb-24 pt-4 px-4 space-y-6">
        <div className="bg-card p-4 rounded-xl border border-border shadow-card flex gap-4 items-center animate-pulse">
  ...
  return (
    <div className="pb-24 pt-4 px-4">
      ...
      <div onClick={() => setCurrentView('view-profile')} className="cursor-pointer transition-transform hover:scale-[1.01] active:scale-[0.99]">
        <AgriCard className="mb-6 overflow-hidden border-2 border-primary/20 hover:border-primary/40 shadow-md">
          <div className="relative overflow-hidden">
            <div className="absolute -right-10 -top-14 h-36 w-36 rounded-full bg-primary/10 blur-2xl" aria-hidden="true" />
            <div className="relative flex items-center gap-4">
              <div className="w-16 h-16 bg-primary rounded-full flex items-center justify-center text-white text-2xl font-semibold shrink-0 shadow-sm">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded bg-primary/10 text-primary text-xs font-semibold uppercase tracking-wider border border-primary/20 shrink-0">
                    Verified ID
                  </span>
                  )}
                </div>
            ...
            <div className="relative mt-4 grid grid-cols-3 gap-2 rounded-xl border border-border/70 bg-background/50 p-2.5">
              {[
                ...
                <div key={s.label} className={cn("text-center", i === 1 && "border-x border-border/50")}>
                  <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{s.label}</p>
                  <p className="text-[12px] font-semibold text-foreground truncate mt-0.5">{s.value}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {menuSections.map((section) => (
        <div key={section.title} className="mb-5">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground px-1 mb-2">
            {section.title}
          </p>
          <div className="space-y-2">
            {section.items.map((item, idx) => (
              <button
                ...
                onClick={item.action}
                className="w-full bg-card p-4 rounded-xl border border-border shadow-card flex items-center justify-between hover:shadow-soft hover:border-primary/30 transition-all group"
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block text-xs font-semibold text-muted-foreground truncate">{item.sub}</span>
                  </span>
                </div>
      ...
      <button
        onClick={toggleMute}
        className="w-full bg-card p-4 rounded-xl border border-border shadow-card flex items-center justify-between mb-4 hover:shadow-soft transition-shadow group"
      >
        <div className="flex items-center gap-3 min-w-0">
          {isMuted ? (
      ...
      <button
        ...
        className="w-full border border-border p-3 rounded-xl flex items-center justify-center gap-2 text-xs font-semibold text-muted-foreground hover:text-foreground transition-colors"
      >
        <LogOut size={14} />
        Logout from all devices
      </button>
