...
import {
  ...
  Phone, Clock, Star, Sprout, Wheat, Camera,
} from "lucide-react";
...
const KisanChat: React.FC<KisanChatProps> = ({ onClose, selectedLanguage: propLanguage }) => {
  ...
  const renderPlaceCard = (place: NearbyPlace) => (
    <div
      key={place.id}
      className="rounded-xl border border-border bg-card p-3.5 hover:bg-muted/40 transition-colors"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className={cn(
            "w-9 h-9 rounded-lg flex items-center justify-center shrink-0 bg-muted text-muted-foreground"
          )}>
            {place.type === "market" ? <MapPin size={16} /> : <Store size={16} />}
          </div>
          <div className="min-w-0">
            <p className="type-small font-semibold text-foreground truncate">{place.name}</p>
        ...
        <div className="flex flex-col items-end shrink-0 gap-1">
          {place.distance && (
            <span className="type-meta font-semibold text-foreground bg-muted px-2 py-0.5 rounded whitespace-nowrap">
              {place.distance}
            </span>
          )}
          <span className="type-meta text-muted-foreground font-semibold flex items-center gap-0.5">
      ...
      {place.specialty && (
        <p className="type-meta text-muted-foreground mt-2">{place.specialty}</p>
      )}

      <div className="flex flex-wrap gap-2.5 mt-2 type-meta text-muted-foreground">
      ...
      <button
        ...
        className="mt-3 w-full flex items-center justify-center gap-1.5 type-small font-semibold text-foreground bg-muted hover:bg-muted/70 rounded-lg py-2 transition-colors"
      >
        <Navigation size={12} />
  ...
  const renderAssistantContent = (msg: ChatMessage, isLast: boolean) => {
    ...
    if (msg.structured?.kind === "nearby") {
      return (
        <div className="space-y-3">
          <div className="whitespace-pre-wrap font-semibold text-foreground">{msg.content}</div>
          {!msg.structured.hasLocation && (
            <p className="type-meta text-muted-foreground bg-muted/50 border border-border rounded-lg px-3 py-2">
              {isHindi
          ...
          {isLast && (
            <div className="flex gap-2 pt-1">
              <button
                ...
                className="type-small font-semibold text-foreground bg-card hover:bg-muted border border-border rounded px-3 py-1.5 transition-colors"
              >
    ...
    return (
      <div>
        ...
        {showTyping && (
          <span className="inline-block w-0.5 h-4 align-middle bg-primary ml-0.5 animate-pulse" />
        )}
      </div>
    );
  };
  ...
  return (
    <div className="h-full flex flex-col bg-background text-foreground relative select-none">
      ...
      <div className="bg-card border-b border-border px-4 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-1.5">
          <button
            onClick={onClose}
            className="w-10 h-10 flex items-center justify-center hover:bg-muted rounded-lg transition-colors text-muted-foreground hover:text-foreground"
          ...
          <div className="flex items-center gap-2.5 ml-1">
            ...
            <div>
              ...
              <p className="type-meta flex items-center gap-1.5">
                <span className={cn("w-1.5 h-1.5 rounded-full", isOffline ? "bg-amber-500" : "bg-primary")} />
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-10 h-10 flex items-center justify-center bg-muted hover:bg-muted/70 text-foreground rounded-lg transition-colors border border-border"
          ...
          <button
            ...
            className={cn(
              "w-10 h-10 flex items-center justify-center rounded-lg transition-colors border",
              autoSpeak
                ? "bg-primary text-primary-foreground border-primary"
                : "text-muted-foreground hover:text-foreground hover:bg-muted border-border"
            )}
            title={autoSpeak ? t('chat.autoVoiceOn') : t('chat.autoVoiceOff')}
            aria-label={autoSpeak ? t('chat.autoVoiceOn') : t('chat.autoVoiceOff')}
          >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {isOffline && (
          <div className="absolute top-0 left-0 right-0 bg-amber-500/10 text-amber-800 dark:text-amber-300 text-center py-1.5 px-3 z-30 flex items-center justify-center gap-2 type-small font-semibold border-b border-amber-500/25">
        ...
        <div
          className={cn(
            "absolute inset-y-0 left-0 w-[260px] bg-card border-r border-border z-40 transition-transform duration-300 transform",
            showHistory ? "translate-x-0" : "-translate-x-full"
          )}
        >
          <div className="p-3 border-b border-border flex items-center justify-between">
            <span className="type-label text-muted-foreground">{t('agr218')}</span>
            <button
              onClick={() => setShowHistory(false)}
              className="w-8 h-8 flex items-center justify-center hover:bg-muted rounded-md text-muted-foreground hover:text-foreground transition-colors"
          ...
          <div className="p-2 space-y-1 overflow-y-auto max-h-[85vh] no-scrollbar">
            {sessions.length === 0 ? (
              <p className="type-small text-muted-foreground p-3 text-center">{t('agr219')}</p>
            ) : (
              sessions.map(s => (
                <div
                  key={s.id}
                  ...
                  className={cn(
                    "flex items-center justify-between p-2.5 rounded-lg cursor-pointer transition-colors border type-small",
                    currentSessionId === s.id
                      ? "bg-primary/5 border-primary/25 text-foreground font-semibold"
                ...
                >
                  ...
                  <button
                    ...
                    className="hover:text-destructive p-2 hover:bg-muted rounded-md transition-colors shrink-0 ml-1"
        ...
        <div className="flex-1 flex flex-col justify-between overflow-y-auto no-scrollbar relative">
                ...
                <div className="flex flex-wrap justify-center items-center gap-1.5">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className="type-small bg-card border border-border rounded px-3 py-1.5 text-foreground hover:bg-muted transition-colors"
                    >
                      {prompt}
                    </button>
                  ))}
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-2.5 max-w-[92%] sm:max-w-[85%]">
                    ...
                    <div
                      className={cn(
                        "p-3.5 rounded-xl text-xs sm:text-sm transition-colors leading-relaxed",
                        msg.role === "user"
                    ...
                    >
                      ...
                      {msg.image && (
                        <div className="mb-3 max-w-[200px] rounded-lg overflow-hidden border border-border">
                          <SafeImage
                            src={msg.image}
                            alt="Attached crop"
                            resolveType="crop"
                      ...
                      {msg.role === "assistant" && !msg.structured && (
                        <div className="flex items-center justify-between gap-3 mt-3.5 pt-2 border-t border-border type-meta">
                          <button
                            onClick={() => handleSpeakMessage(msg.content)}
                            className="hover:text-foreground flex items-center gap-1 transition-colors text-muted-foreground"
                          >
                            <Volume2 size={13} />
                            {isSpeaking && isLast ? t('chat.stopSpeakingBtn') : t('chat.listenBtn')}
                          </button>
                          {msg.source === "local" && (
                            <span className="flex items-center gap-1 type-meta font-semibold text-amber-700 dark:text-amber-300 bg-amber-500/10 border border-amber-500/25 rounded px-2 py-0.5">
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-3 pt-2.5 border-t border-border">
                          {msg.suggestions.map((s, i) => (
                            <button
                              ...
                              className="type-small text-foreground bg-muted hover:bg-muted/70 border border-border rounded px-3 py-1.5 transition-colors"
                            >
                              {s}
                            </button>
                          ))}
            ...
            {isLoading && (
              <div className="flex justify-start items-start gap-2.5">
                ...
                <div className="bg-card border border-border p-3.5 rounded-xl rounded-bl-sm flex items-center gap-1.5">
                  ...
                  <span className="w-1.5 h-1.5 bg-muted-foreground rounded-full " style={{ animationDelay: '150ms' }} />
                  <span className="w-1.5 h-1.5 bg-muted-foreground rounded-full " style={{ animationDelay: '300ms' }} />
                </div>
              </div>
            )}
          ...
          <div className="px-4 pb-1 bg-card border-t border-border">
            <div className="flex gap-2 overflow-x-auto no-scrollbar py-2">
              <button
                onClick={() => fetchNearby("markets")}
                disabled={isLoading}
                className="flex items-center gap-1.5 shrink-0 type-small text-foreground bg-card hover:bg-muted border border-border rounded px-3 py-1.5 transition-colors disabled:opacity-40"
              >
                <MapPin size={12} aria-hidden="true" /> {t('chat.qaMandi')}
              </button>
              <button
                ...
                className="flex items-center gap-1.5 shrink-0 type-small text-foreground bg-card hover:bg-muted border border-border rounded px-3 py-1.5 transition-colors disabled:opacity-40"
              >
                <Store size={12} aria-hidden="true" /> {t('chat.qaShops')}
              </button>
          ...
          <div className="p-4 bg-card border-t border-border space-y-3">
            ...
            {imagePreview && (
              <div className="flex items-center justify-between p-2.5 bg-muted/50 border border-border rounded-lg max-w-xs">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <span className="type-label text-muted-foreground">{t('agr221')}</span>
                    <p className="type-meta truncate max-w-[120px]">{t('agr222')}</p>
                  </div>
                </div>
                <button
                  onClick={clearAttachedImage}
                  className="w-8 h-8 flex items-center justify-center bg-card hover:bg-destructive/10 hover:text-destructive rounded-md transition-colors text-muted-foreground border border-border"
            ...
            <div className="flex items-center gap-2 relative">
              ...
              <button
                ...
                className={cn(
                  "w-11 h-11 flex items-center justify-center rounded-lg transition-colors border shrink-0",
                  imagePreview
                    ? "bg-primary/10 text-primary border-primary/30"
                    : "bg-card text-muted-foreground hover:bg-muted border-border"
                )}
                title="Attach a crop leaf photo for diagnosis"
                aria-label="Attach crop leaf image"
              >
                <Image size={18} />
              </button>
              ...
              <button
                ...
                aria-label={t("chat.voiceInputTitle")}
              >
                <Mic size={18} />
              </button>
              ...
              <input
                ...
                className="flex-1 h-11 px-4 bg-background rounded-lg text-base sm:text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring border border-border text-foreground placeholder:text-muted-foreground transition-colors"
                value={input}
              ...
              <button
                ...
                className="w-11 h-11 flex items-center justify-center bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 disabled:opacity-30 disabled:pointer-events-none transition-colors shrink-0"
