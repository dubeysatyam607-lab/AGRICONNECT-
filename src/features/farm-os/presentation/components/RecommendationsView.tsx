        ...
        <>
          <div className="flex flex-col gap-2">
            {pending.map((r) => {
              ...
              return (
                <article key={r.id} className="rounded-xl border border-border bg-card p-3.5 shadow-card">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className={cn('shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold uppercase tracking-wide', priorityStyles[r.priority])}>
                          {t(`fos.rec.priority.${r.priority}`)}
                        </span>
                      </div>
                      <p className="mt-1 text-xs font-medium leading-relaxed text-muted-foreground">{r.body}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-2.5 flex flex-wrap gap-1.5">
                    ...
                    {r.reasons.slice(0, 2).map((reason, i) => (
                      <span key={i} className="rounded-full bg-muted px-2 py-0.5 text-xs font-bold text-muted-foreground">
                  ...
                  <button
                    ...
                    className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-xl bg-forest py-2 text-xs font-semibold text-primary-foreground transition-colors hover:opacity-90"
                  >
          ...
          {done.length > 0 && (
            <section>
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t('fos.rec.done')}</h3>
              <div className="flex flex-col gap-2">
                {done.map((r) => (
                  <div key={r.id} className="flex items-center gap-2.5 rounded-xl border border-border bg-card/60 px-3 py-2.5 opacity-70">
