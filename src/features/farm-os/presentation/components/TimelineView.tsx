    ...
    <div className="mt-4 flex flex-col gap-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="shrink-0 rounded-full bg-muted px-1.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                          {t(`fos.tltype.${e.type}`)}
                        </span>
                      </div>
                      {e.detail && <p className="mt-0.5 text-xs font-medium leading-relaxed text-muted-foreground">{e.detail}</p>}
                      <div className="mt-1 flex items-center justify-between">
                        <span className="text-xs font-bold text-muted-foreground">{formatDay(e.date)}</span>
                        {e.amount != null && (
                          <span className="text-xs font-semibold text-foreground">₹{e.amount.toLocaleString('en-IN')}</span>
                        )}
                      </div>
                    </div>
                  </div>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4" onClick={() => setOpen(false)}>
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <h3 className=" text-sm font-semibold tracking-tight text-foreground">{t('fos.timeline.formTitle')}</h3>
            ...
            <button
              onClick={submit}
              disabled={!title.trim()}
              className="w-full rounded-xl bg-forest py-2.5 text-sm font-semibold text-primary-foreground transition-colors disabled:opacity-40"
            >
        ...
        <button
          ...
          className="rounded-xl border border-dashed border-emerald-300 bg-emerald-500/5 py-3 text-sm font-semibold text-emerald-700 transition-colors hover:bg-emerald-500/10 dark:text-emerald-300"
        >
          + {t('fos.timeline.log')}
        </button>
      )}
