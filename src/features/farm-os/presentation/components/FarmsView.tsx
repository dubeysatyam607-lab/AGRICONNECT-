  ...
  return (
    <div className="mt-4 flex flex-col gap-4">
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isActive ? 'border-emerald-400 bg-emerald-700' : 'border-border bg-card',
                )}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <h4 className="truncate  text-[15px] font-semibold tracking-tight text-foreground">{f.name}</h4>
                      {isActive && (
                        <span className="flex shrink-0 items-center gap-1 rounded-full bg-emerald-500/15 px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">
                ...
                {!isActive && (
                  <button
                    ...
                    className="mt-3 w-full rounded-xl border border-emerald-300 bg-emerald-500/5 py-2 text-xs font-semibold text-emerald-700 transition-colors hover:bg-emerald-500/10 dark:text-emerald-300"
                  >
                    {t('fos.farms.switch')}
                  </button>
                )}
      ...
      <button
        ...
        className="flex items-center justify-center gap-1.5 rounded-xl border border-dashed border-emerald-300 bg-emerald-500/5 py-3 text-sm font-semibold text-emerald-700 transition-colors hover:bg-emerald-500/10 dark:text-emerald-300"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-4 " onClick={(ev) => ev.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <h3 className=" text-sm font-semibold tracking-tight text-foreground">{t('fos.farms.formTitle')}</h3>
            ...
            <button
              ...
              className="mt-4 w-full rounded-xl bg-forest py-2.5 text-sm font-semibold text-primary-foreground disabled:opacity-40"
            >
              {t('fos.farms.submit')}
            </button>
          </div>
