    ...
    <Dialog open={open} onOpenChange={(v) => (v ? reset() : onClose())}>
      <DialogContent className="max-w-md">
        ...
        <div className="mb-3 flex items-center justify-between">
          <span className="text-sm font-semibold text-foreground">{t('pay.secureCheckout')}</span>
        ...
        {step === 'method' && (
          <div className="space-y-4">
            ...
            />
            <div>
              <p className="mb-2 text-xs font-semibold text-muted-foreground">{t('pay.selectMethod')}</p>
            ...
            {method === 'wallet' && walletBalance < total && (
              <p className="text-center text-xs font-bold text-red-600">{t('pay.insufficientWallet')}</p>
            )}
          </div>
        )}
        ...
        {step === 'auth' && (
          <div className="space-y-4">
            <button onClick={() => setStep('method')} className="inline-flex items-center gap-1 text-xs font-bold text-muted-foreground hover:text-foreground">
                      ...
                      <div className="space-y-1.5 flex-1">
                        ...
                        <InputOTP value={otp} onChange={setOtp} maxLength={4} render={({ slots }) => (
                          <InputOTPGroup className="gap-2">
                            {slots.map((slot, i) => (
                              <InputOTPSlot key={i} index={i} {...slot} className="h-11 w-9 rounded-xl border text-lg font-semibold" />
                            ))}
                          </InputOTPGroup>
                        )} />
                      </div>
                    </div>
                    <p className="text-xs font-semibold text-muted-foreground">{t('pay.otpHint')}</p>
                  </div>
                )}
            ...
            {method === 'card' && (
              <div className="space-y-3">
                ...
                <p className="rounded-lg bg-muted/60 px-3 py-2 text-xs font-semibold text-muted-foreground">
            ...
            {method === 'wallet' && (
              <div className="space-y-3">
                <div className="rounded-xl border border-border bg-muted/40 p-4 text-center">
                  <p className="text-xs font-bold text-muted-foreground">{t('pay.walletBalance')}</p>
                  <p className="text-2xl font-semibold text-foreground">{fmtMoney(walletBalance)}</p>
                  <p className="mt-1 text-xs font-semibold text-muted-foreground">
                    {t('pay.paying')} <span className="font-semibold text-foreground">{fmtMoney(total)}</span>
                  </p>
                </div>
                {error && <p className="text-xs font-bold text-red-600">{error}</p>}
                <Button className="w-full" onClick={() => run(false)} disabled={!canPay}>
        ...
        {step === 'processing' && (
          <div className="flex flex-col items-center py-10 text-center">
            ...
            <p className="mt-4 text-sm font-semibold text-foreground">{t('pay.processing')}</p>
            <p className="mt-1 text-xs font-semibold text-muted-foreground">{t('pay.doNotClose')}</p>
          </div>
        )}
        ...
        {step === 'success' && txn && (
          <div className="flex flex-col items-center py-6 text-center">
            ...
            <h3 className="mt-4 text-lg font-semibold text-foreground">{t('pay.successTitle')}</h3>
            <p className="mt-1 text-xs font-semibold text-muted-foreground">{txn.description}</p>
            <p className="mt-3 text-2xl font-semibold text-foreground">{fmtMoney(txn.total)}</p>
            <div className="mt-4 w-full space-y-1.5 rounded-xl bg-muted/40 p-3 text-left text-xs">
              <div className="flex justify-between"><span className="font-semibold text-muted-foreground">{t('pay.transactionId')}</span><span className="font-semibold text-foreground">{txn.id}</span></div>
              <div className="flex justify-between"><span className="font-semibold text-muted-foreground">{t('pay.gateway')}</span><span className="font-semibold uppercase text-foreground">{txn.gateway}</span></div>
              {txn.gatewayRef && <div className="flex justify-between"><span className="font-semibold text-muted-foreground">Ref</span><span className="font-semibold text-foreground">{txn.gatewayRef}</span></div>}
            </div>
        ...
        {step === 'failed' && (
          <div className="flex flex-col items-center py-6 text-center">
            ...
            <h3 className="mt-4 text-lg font-semibold text-foreground">{t('pay.failedTitle')}</h3>
            <p className="mt-1 max-w-xs text-xs font-semibold text-muted-foreground">{error}</p>
            <p className="mt-2 rounded-full bg-muted px-3 py-1 text-xs font-semibold text-muted-foreground">
