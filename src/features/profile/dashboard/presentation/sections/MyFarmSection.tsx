...
export const MyFarmSection: React.FC<MyFarmSectionProps> = ({ data }) => {
 ...
 <h2 className="text-xl font-semibold text-foreground tracking-tight">{t('prof.myFarm')}</h2>
 <button
 onClick={() => setEditing(true)}
 className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground hover:bg-primary/90 active:scale-95 transition-all"
 >
 ...
 {/* Farm summary hero */}
 <div className="relative rounded-xl bg-emerald-900 text-white overflow-hidden p-6">
 ...
 <div>
 <p className="text-lg font-semibold tracking-tight">{data.farmExtras.farmName}</p>
 ...
 <p className="text-lg font-semibold tabular-nums">{farmArea}</p>
 <p className="text-xs font-bold text-emerald-100 uppercase tracking-wider">{t('prof.farmSize')}</p>
 </div>
 <div className="rounded-xl bg-white/10 p-3">
 <p className="text-lg font-semibold">{data.onboarding.primaryCrops[0]?.split(' (')[0] || t('prof.na')}</p>
 <p className="text-xs font-bold text-emerald-100 uppercase tracking-wider">{t('prof.primaryCrop')}</p>
 </div>
 <div className="rounded-xl bg-white/10 p-3">
 <p className="text-lg font-semibold tabular-nums">{daysToHarvest}d</p>
 <p className="text-xs font-bold text-emerald-100 uppercase tracking-wider">{t('prof.untilHarvest')}</p>
 </div>
 </div>
 </div>
 ...
 <h3 className="text-sm font-semibold text-foreground mb-2">{t('prof.farmDetails')}</h3>
 ...
 <span className="text-xs font-semibold text-foreground">{t('prof.locationPerm')}</span>
 <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wider ${data.profile?.location.isLocationPermissionGranted ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-muted text-muted-foreground'}`}>
 {data.profile?.location.isLocationPermissionGranted ? t('prof.locationOn') : t('prof.locationOff')}
 </span>
 </p>
 <p className="mt-1.5 text-xs text-muted-foreground leading-relaxed">{t('prof.locationPermHint')}</p>
 </div>

 <SheetFooter className="mt-6 flex gap-3 sm:justify-end">
 <button onClick={() => setEditing(false)} className="rounded-full px-5 py-2.5 text-xs font-semibold text-muted-foreground hover:bg-muted transition-colors">
 {t('prof.cancel')}
 </button>
